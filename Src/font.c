...
static const uint64_t font8x8[128] = {
...
static void toggleCursor(){
	int yoffset = START_LINE + y_pos*CHAR_HEIGHT;
	for(int y = yoffset; y < yoffset+CHAR_HEIGHT; y++){
		TV.buffer[y][x_pos] = ~TV.buffer[y][x_pos];
	}
}

static void TV_NextLine() {
	if(y_pos < CHAR_COUNT_Y - 1) { TV_GoToXY(0, y_pos + 1); }
}

void TV_CursorOn(){
	if(!showCursor){
		showCursor = true;
		toggleCursor(); //dispaly cursor
	}
}
...
void TV_GoToXY(int x, int y){
	...
	x_pos = max(0, min(x, BUFF_SIZE_X - 1));
	y_pos = max(0, min(y, CHAR_COUNT_Y - 1));
	if(showCursor){
		toggleCursor(); //draw cursor
	}
}
...
void TV_PrintChar(char ch){
	...
	for(int y = 0; y < CHAR_HEIGHT; y++){
		TV.buffer[y + yoffset][x_pos] = ((uint8_t*)&(font8x8[(uint8_t)ch]))[CHAR_HEIGHT - 1 - y];
	}
	x_pos += 1;
	if(x_pos >= BUFF_SIZE_X){
		x_pos = 0;
		y_pos = min(y_pos + 1, CHAR_COUNT_Y - 1);
	}
	if(showCursor){
		toggleCursor();
	}
...
int TV_PrintLine(const char *text){
	const int max_length = BUFF_SIZE_X - x_pos;
	bool endline = false;
	int count = 0;
	while(text[count] && count <= max_length){
		if(text[count] == '\n'){
			endline = true;
			break;
		}
	...
	for(int j = 0; j < CHAR_HEIGHT; j++){
		for(int i = 0; i < count; i++){
			TV.buffer[y_pos*CHAR_HEIGHT + j][i + x_pos] = 
				((uint8_t*)&(font8x8[(uint8_t)text[i]]))[CHAR_HEIGHT - 1 - j];
		}
	}
	if(endline){
		TV_NextLine();
	} else {
		TV_GoToXY(x_pos + count, y_pos);
	}
	return count;
}
...
int TV_PrintText(const char *text){
	int printed = 0;
	const int max_length = min(CHAR_COUNT_Y * BUFF_SIZE_X - (y_pos * BUFF_SIZE_X + x_pos), _strlen(text));
	while((printed += TV_PrintLine(text + printed)) < max_length);
	return printed;
}